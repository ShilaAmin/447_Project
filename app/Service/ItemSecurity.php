...
/**
 * Item listing feature (create / view / edit / search).
 * Algorithm: from-scratch ECC NIST P-256 (ECIES: C1=kG, S=kQ, C2=M XOR stream). HMAC for integrity.
 */
class ItemSecurity
{
    ...
    {
        $pub = $this->keys->eccPublic();
        $encrypted = [
            'title' => $this->ecc->encrypt($fields['title'], $pub),
            'description' => $this->ecc->encrypt($fields['description'], $pub),
            'preferred_product' => $this->ecc->encrypt($fields['preferred_product'], $pub),
        ];
        $encrypted['mac'] = $this->generateItemMac($encrypted);

        return $encrypted;
    }

    public function decryptFields(Item $item): array
    {
        $this->assertItemIntegrity($item);

        return [
            'title' => $this->decrypt($this->stored($item, 'title') ?? ''),
            'description' => $this->decrypt($this->stored($item, 'description') ?? ''),
            'preferred_product' => $this->decrypt($this->stored($item, 'preferred_product') ?? ''),
        ];
    }

    public function encryptImageMeta(array $meta): array
    {
        $ciphertext = $this->ecc->encrypt(json_encode($meta, JSON_THROW_ON_ERROR), $this->keys->eccPublic());

        return [
            'image_meta' => $ciphertext,
            'image_meta_mac' => $this->mac->generate($ciphertext),
        ];
    }

    public function decryptImageMeta(Item $item): ?array
    {
        $imageMeta = $this->stored($item, 'image_meta');
        if (!$imageMeta) {
            return null;
        }

        $imageMetaMac = $this->stored($item, 'image_meta_mac');
        if (!$imageMetaMac || !$this->mac->verify($imageMeta, $imageMetaMac)) {
            throw new RuntimeException('Image metadata integrity check failed.');
        }

        $json = $this->decrypt($imageMeta);
        $meta = json_decode($json, true, 512, JSON_THROW_ON_ERROR);

        return is_array($meta) ? $meta : null;
    }

    public function generateItemMac(array $encryptedFields): string
    {
        return $this->mac->generate($this->mac->join([
            $encryptedFields['title'] ?? '',
            $encryptedFields['description'] ?? '',
            $encryptedFields['preferred_product'] ?? '',
        ]));
    }

    public function verifyItemMac(Item $item): bool
    {
        $mac = $this->stored($item, 'mac');
        if (!$mac) {
            return false;
        }

        return $this->mac->verify($this->mac->join([
            $this->stored($item, 'title') ?? '',
            $this->stored($item, 'description') ?? '',
            $this->stored($item, 'preferred_product') ?? '',
        ]), $mac);
    }

    public function assertItemIntegrity(Item $item): void
    {
        if (!$this->verifyItemMac($item)) {
            throw new RuntimeException('Item integrity check failed.');
        }
    }

    public function hydrateDecrypted(Item $item): ?Item
    {
        try {
            $plain = $this->decryptFields($item);
            $item->setAttribute('title', $plain['title']);
            $item->setAttribute('description', $plain['description']);
            $item->setAttribute('preferred_product', $plain['preferred_product']);
            return $item;
        } catch (\Throwable) {
            return null;
        }
    ...
    public function hydrateTitle(Item $item): ?Item
    {
        try {
            $this->assertItemIntegrity($item);
            $item->setAttribute('title', $this->decrypt($this->stored($item, 'title') ?? ''));
            return $item;
        } catch (\Throwable) {
            $item->setAttribute('title', '[integrity failed]');
            return $item;
        }
    }
    ...
    {
        return $this->ecc->decrypt($ciphertext, $this->keys->resolveEccPrivateFromCipher($ciphertext));
    }

    private function stored(Item $item, string $field): ?string
    {
        $originals = $item->getRawOriginal();
