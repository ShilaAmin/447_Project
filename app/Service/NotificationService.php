 ...
 * Algorithm: from-scratch ECC NIST P-256 (ECIES in this file as NotificationEcc). HMAC for integrity.
 */
class NotificationService
{
    public function __construct(
        private NotificationEcc $ecc,
    ...
    public function push(int $userId, string $plaintext): Notification
    {
        $encrypted = $this->ecc->encrypt($plaintext, $this->keys->eccPublic());

        return Notification::create([
            'user_id' => $userId,
            'message' => $encrypted,
            'mac' => $this->mac->generate($encrypted),
            'read' => 0,
        ]);
    }

    public function decrypt(Notification $notification): string
    {
        $message = (string) $notification->getRawOriginal('message') ?: (string) $notification->message;
        $mac = (string) ($notification->getRawOriginal('mac') ?? $notification->mac ?? '');

        if ($mac === '' || !$this->mac->verify($message, $mac)) {
            throw new RuntimeException('Notification integrity check failed.');
        }

        return $this->ecc->decrypt($message, $this->keys->resolveEccPrivateFromCipher($message));
    }

    public function hydrate(Notification $notification): ?Notification
    {
        try {
            $plain = $this->decrypt($notification);
            $notification->setAttribute('message', $plain);
            return $notification;
        } catch (\Throwable) {
            return null;
        }
